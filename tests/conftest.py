        ...
        for session in sessions:
            ...
            if any([i for i in ['all', metafunc.function.__name__] if i in session['scopes']]):
                ...
                if 'account' in metafunc.fixturenames:
                    ...
                    test_data.append(len(active_session_based_fixtures) and ([session]*(len(active_session_based_fixtures) + 1)) or session)
        # TODO update ids generation
        metafunc.parametrize(
            ','.join([i for i in ['session'] + list(active_session_based_fixtures) + ['account', 'account_currency'] if i in metafunc.fixturenames]),
...
def apikey(request):
    ...
    response = requests.post(f'{trading_url}/api/auth/apikeys/new',
                             ...
                             cookies=session['cookies'], json={"scopes": ["trading", "transfers"], "accounts": get_accounts(session)})
