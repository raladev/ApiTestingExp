...
class TestProfilePositive:
        ...
        def test_post_affiliate_enroll(self, session):
                response = requests.post(f'{trading_url}/api/profile/affiliate/enroll', headers={'X-Auth-Nonce': session['nonce']},
                                      cookies=session['cookies'])
                logging.info(response.headers)
                logging.info(response.text)
                assert response.status_code == 200
        ...
        def test_post_create_link(self, session):
                response = requests.post(f'{trading_url}/api/profile/affiliate/create_link', headers={'X-Auth-Nonce': session['nonce']},
                                      cookies=session['cookies'])
                logging.info(response.headers)
                logging.info(response.text)
                assert response.status_code == 200
        ...
        def test_get_affiliate_links(self, session):
                response = requests.get(f'{trading_url}/api/profile/affiliate/links', headers={'X-Auth-Nonce': session['nonce']},
                                      cookies=session['cookies'])
                logging.info(response.headers)
                logging.info(response.text)
                assert response.status_code == 200
        ...
        def test_get_affiliate_referrals(self, session):
                response = requests.get(f'{trading_url}/api/profile/affiliate/referrals', headers={'X-Auth-Nonce': session['nonce']},
                                      cookies=session['cookies'])
                logging.info(response.headers)
                logging.info(response.text)
                assert response.status_code == 200
